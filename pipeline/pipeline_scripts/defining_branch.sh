...
#Определяем VLE_SOLVERS_BRANCH (ветку в репозитории VLE_SOLVERS)
if [ "$CI_PIPELINE_SOURCE" == "merge_request_event" ]; then
	export VLE_SOLVERS_BRANCH="$CI_MERGE_REQUEST_SOURCE_BRANCH_NAME"
else
	export VLE_SOLVERS_BRANCH="$CI_COMMIT_BRANCH"
fi
...
defining_branch() {
	...
	local mathfold_branch="math/$target_branch" # Доп. переменная для поиска ветки в папке
...
defining_branch "$FIXED_SOLVERS_BRANCHES" "$VLE_SOLVERS_BRANCH" "FIXED_SOLVERS_BRANCH" "$DEFAULT_FIXED_SOLVERS_BRANCH"
# Записываем название ветки в репозитории VLE_SOLVERS в файл $filename
echo "VLE_SOLVERS_BRANCH=$VLE_SOLVERS_BRANCH" >> "$filename"
